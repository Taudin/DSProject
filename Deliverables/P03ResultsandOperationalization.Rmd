...
Next, we need to transform the vector of predicted probabilities $\hat{π}=(\hat{π}_1,\hat{π}_2,…,\hat{π}_n) (n=30162)$ into a binary vector of predicted responses (predicted income). We denote this binary vector with $\mathbf{\hat{y}}=(\hat{y}_1,\hat{y}_2,\dots,\hat{y}_n)$. We construct the vector $\hat{y}$ in the following way:

$$\hat{y}_i=1,\hspace{0.5cm}if\hspace{0.5cm} \hat{π}_i>0.5,$$
and
...
What we have here is a logistic model that can predict whether an American citizen earns over $50,000 per year rather accurately.  Type of tool may be used in a broad range of operational applications: Retail, finance, tourism, and real estate just to name a few.  As its geographical orientation could change, the model may be adjusted accordingly to account for varying demographics between regions. 

Furthermore, other models that could be built on this data are random forest, support vector machine, XGBoost, and neural networks. The data set is such that it would be interesting to explore and compare results provided by each of these other machine learning algorithms.

## Ethical Concerns

Being that this model is to predict whether or not an individual's annual income exceeds a certain threshold, it is bound to what the cold hard data that feeds it.  As discussed earlier in Part 02 we see that certain groups of people are more likely to earn more than $50,000 annually while others are likely to not earn as much as that.  This doesn't necessarily have to be interpreted as a negative characteristic of what a predictive model like this can do and any implications of what that means. Models like these can help us identify and meet the concerns, interests, or needs of both groups expressed as the binary classification of this model, and generally advise us in making nuanced decisions, being careful to avoid wholesale prejudice that may deny up and comers any positive opportunities.