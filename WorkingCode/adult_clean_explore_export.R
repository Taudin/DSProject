...
adult_data <- read.table("CensusData/adult.data", sep = ",", header = FALSE)

#Preliminary look at the data.
#The number of observations in the adult_data dataframe is:
dim(adult_data)[1]
...
colnames(adult_data) <- c("age", "workclass", "fnlwgt", "education", "education_num", "marital_status", "occupation", "relationship",
...
#Its structure:
str(adult_data, vec.len = 2, strict.width = "no", width = 30)
...
#In order to see what the levels of each factor variable are, we will provide a function "levels_factors()", which takes a dataframe as
...
adult_data <- read.table("CensusData/adult.data", sep = ",", header = FALSE, na.strings = " ?")
...
#We  will group the working hours in 5 categories considered relevant. We will also create a new factor variable called "hours_w" with 
#5 levels corresponding to these 5 categories.
adult_data$hours_w[adult_data$hours_per_week < 40] <- " less_than_40"
adult_data$hours_w[adult_data$hours_per_week >= 40 & adult_data$hours_per_week <= 45] <- " between_40_and_45"
adult_data$hours_w[adult_data$hours_per_week > 45 & adult_data$hours_per_week <= 60] <- " between_45_and_60"
adult_data$hours_w[adult_data$hours_per_week > 60 & adult_data$hours_per_week <= 80] <- " between_60_and_80"
adult_data$hours_w[adult_data$hours_per_week > 80] <- " more_than_80"
...
#We make the new variable "hours_w" a factor variable, we will use the "factor()" function to do so.
adult_data$hours_w <- factor(adult_data$hours_w, ordered = FALSE, levels = c(" less_than_40", " between_40_and_45", " between_45_and_60",
                                                                             " between_60_and_80", " more_than_80"))

#We can see how many people belong to each category of the factor variable "hours_w":
...
levels(adult_data$native_country)
...
adult_data <- mutate(adult_data, native_region = ifelse(native_country %in% Asia_East, "East-Asia",
...
summary(adult_data$capital_gain)
summary(adult_data$capital_loss)

#The percentage of zeros in "capital_gain is 91.59%!
(nrow(subset(adult_data, adult_data$capital_gain == 0)) / nrow(adult_data)) * 100

#The percentage of zeros in "capital_loss" is also very high...
(nrow(subset(adult_data, adult_data$capital_loss == 0)) / nrow(adult_data)) * 100
...
mean_gain <- mean(adult_data$capital_gain)
mean_loss <- mean(adult_data$capital_loss)
...
mean_gain <- mean(subset(adult_data$capital_gain, adult_data$capital_gain > 0))
mean_loss <- mean(subset(adult_data$capital_loss, adult_data$capital_loss > 0))
...
iqr_gain <- IQR(subset(adult_data$capital_gain, adult_data$capital_gain > 0))
iqr_loss <- IQR(subset(adult_data$capital_loss, adult_data$capital_loss > 0))
quantile_gain <- quantile(x = subset(adult_data$capital_gain, adult_data$capital_gain > 0), probs = seq(0, 1, 0.25))
quantile_loss <- quantile(x = subset(adult_data$capital_loss, adult_data$capital_loss > 0), probs = seq(0, 1, 0.25))
...
df <- adult_data[adult_data$capital_gain > 0,]
...
ggplot(aes(x = factor(0), y = capital_loss), data = subset(adult_data, adult_data$capital_loss > 0)) +
  geom_boxplot() +
  stat_summary(fun.y = mean, geom = "point", shape = 19, color = "red", cex = 2) +
  scale_x_discrete(breaks = NULL) +
...
df <- adult_data[adult_data$capital_loss > 0,]
...
adult_data <- mutate(adult_data, cap_gain = ifelse(adult_data$capital_gain < 3464, "Low",
                                            ifelse(adult_data$capital_gain >= 3464 & adult_data$capital_gain <= 14080, "Medium", "High")))
adult_data$cap_gain <- factor(adult_data$cap_gain, ordered = TRUE, levels = c("Low", "Medium", "High"))

adult_data <- mutate(adult_data, cap_loss = ifelse(adult_data$capital_loss< 1672, "Low",
                                            ifelse(adult_data$capital_loss >= 1672 & adult_data$capital_loss <= 1977, "Medium","High")))
adult_data$cap_loss <- factor(adult_data$cap_loss, ordered = TRUE, levels = c("Low", "Medium", "High"))
...
#We repeat the same steps as in the transformation of the training dataframe "adult_data".
...
#from the respective names in the training data "adult_data" by the symbol ".". We remove the "." from the names of the factor levels
#of "income" in the test data.
levels(adult_test$income)[1] <- "<=50K"
levels(adult_test$income)[2] <- ">50K"
levels(adult_test$income)

#Just like the training data we create a new variable called "hours_w".
adult_test$hours_w[adult_test$hours_per_week < 40] <- "less_than_40"
adult_test$hours_w[adult_test$hours_per_week >= 40 & adult_test$hours_per_week <= 45] <- "between_40_and_45"
adult_test$hours_w[adult_test$hours_per_week > 45 & adult_test$hours_per_week <= 60] <- "between_45_and_60"
adult_test$hours_w[adult_test$hours_per_week > 60 & adult_test$hours_per_week <= 80] <- "between_60_and_80"
adult_test$hours_w[adult_test$hours_per_week > 80] <- "more_than_80"

adult_test$hour_w <- factor(adult_test$hours_w, ordered = FALSE,
...
write.csv(adult_data, "adult_df.csv", row.names = FALSE)
